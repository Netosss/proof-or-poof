...
def get_system_instruction(quality_context: str) -> str:
    ...
    * Look ONCE for a VISIBLE, HUMAN-READABLE AI-generation marker at a normal viewing scale — e.g. a "Generated by AI" caption, a DALL·E rainbow color strip in the bottom-right corner, a visible "CR" (Content Credentials) icon, or a clearly legible Midjourney / Sora / Adobe Firefly badge.
    * Do NOT attempt to detect SynthID, cryptographic watermarks, or any invisible-by-design provenance signal — those require a dedicated cryptographic decoder and cannot be read from pixels. Do NOT hunt pixel-by-pixel for tiny icons; if a marker is not plainly visible, treat it as absent and move on.
