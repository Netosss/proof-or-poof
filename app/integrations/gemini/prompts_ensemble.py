...
def get_combined_prompt(quality_context: str) -> str:
    """
    Single-call 'batch' prompt — merges anatomy + physics + composition
    perspectives into one system instruction. Returns one consolidated
    verdict (EnsembleSubResult shape) covering all three failure classes.

    Used for cost/accuracy benchmarking against the parallel 3-voter
    ensemble: comparable detection surface, 1× cost instead of 3×.
    """
    return f"""<Persona>
