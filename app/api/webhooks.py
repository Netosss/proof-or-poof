...
async def lemonsqueezy_webhook(
    ...
    logger.info(
        "lemonsqueezy_webhook_arrived",
        extra={
            "action": "lemonsqueezy_webhook_arrived",
    ...
    if not x_signature or not hmac.compare_digest(x_signature, expected_signature):
        logger.error(
            ...
            extra={
                "action": "lemonsqueezy_signature_invalid",
                "has_signature": x_signature is not None,
                "ip": request.client.host if request.client else "",
            },
        )
    ...
    logger.info(
        "lemonsqueezy_webhook_parsed",
        extra={
            "action": "lemonsqueezy_webhook_parsed",
            "event_name": event_name,
            "order_id": order_id,
            "test_mode": meta.get("test_mode"),
            "custom_user_id": custom_data.get("user_id", ""),
            "custom_env": custom_data.get("env", ""),
        },
    )
    ...
    if event_name == "order_created":
        ...
        if order_status != "paid":
            logger.info(
                "lemonsqueezy_order_created_skipped",
                extra={
...
async def _handle_order_paid(payload, meta, data, attributes, order_id, custom_data):
    ...
    if not user_id:
        logger.error(
            "lemonsqueezy_order_paid_missing_user",
            extra={
    ...
    if not credits:
        logger.error(
            "lemonsqueezy_variant_not_found",
            extra={
    ...
    if not db:
        logger.error(
            "lemonsqueezy_firestore_unavailable",
            extra={
...
async def _handle_order_refunded(order_id: str):
    ...
    try:
        ...
        logger.error(
            "lemonsqueezy_refund_deduction_failed",
            extra={
