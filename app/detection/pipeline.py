...
# Long substring tokens — safe to match via `in` against agent / generator strings
# without word-boundary checks. Long enough that accidental collisions are very unlikely.
_KNOWN_AI_C2PA_GENERATORS_LONG = {
    # OpenAI / ChatGPT
    ...
    "google media processing services", "imagen", "google deepmind", "gemini",
    # Amazon Titan
    ...
    # Other notable AI tools (long forms)
    "ideogram", "leonardo", "haiper", "dream machine", "runway",
    # Late-2024 / 2025 entrants
    "meta ai", "imagine with meta", "bing image creator", "microsoft designer",
    "recraft", "grok", "xai",
}

# Short tokens (≤5 chars) — require WORD-BOUNDARY match to avoid substring
# collisions (e.g. "sora" matching "Sorano Photo Editor"). Built into a regex.
_KNOWN_AI_C2PA_GENERATORS_SHORT = (
    "sora", "flux", "kling", "luma", "pika", "reve",
)
_AI_SHORT_GEN_RE = re.compile(
    r"\b(?:" + "|".join(map(re.escape, _KNOWN_AI_C2PA_GENERATORS_SHORT)) + r")\b",
    re.IGNORECASE,
)


def _has_known_ai_generator(text: str) -> bool:
    """Match against the long set via substring, the short set via word-boundary regex."""
    if not text:
        return False
    low = text.lower()
    if any(g in low for g in _KNOWN_AI_C2PA_GENERATORS_LONG):
        return True
    return bool(_AI_SHORT_GEN_RE.search(text))
...
async def detect_ai_media(file_path: str, trusted_metadata: dict = None) -> dict:
    ...
    if manifest:
        ...
        if not is_generative_ai:
            for assertion in assertions:
                ...
                agent = str(data.get("softwareAgent", ""))
                desc = str(data.get("description", "")).lower()
                if _has_known_ai_generator(agent):
                    is_generative_ai = True
        ...
        if not is_generative_ai:
            for name in generator_names:
                if _has_known_ai_generator(name):
                    is_generative_ai = True
                    logger.info("c2pa_ai_flagged", extra={
                        "action": "c2pa_ai_flagged",
    ...
    if is_video:
        ...
        gemini_result = await analyze_video_frames_async(frames)
        ...
        # Hard-failure guard: -1.0 sentinel from analyze_batch_images_pro_turbo
        # (Gemini network error OR empty/unparseable structured output). Must NOT
        # cache and must NOT surface as a confident verdict — return "Analysis Failed".
        if confidence < 0.0:
            logger.warning("video_gemini_failed", extra={
                "action": "video_gemini_failed",
                "media_file": filename,
            })
            return {
                "summary": "Analysis Failed",
