...
async def detect_ai_media_image_logic(
    ...
    logger.info(f"[DEBUG] meta scores - human: {human_score} ({type(human_score)}), ai: {ai_score} ({type(ai_score)})")
    ...
    gpu_result = await run_deep_forensics(file_path if file_path else Image.fromarray(np.uint8(frame)) if frame is not None else "")
    ...
    if ai_score >= ScoringConfig.THRESHOLDS["CONFLICT_AI_SCORE"] and forensic_probability < ScoringConfig.THRESHOLDS["CONFLICT_MODEL_LOW"]:
        if hard_ai_signal:
    ...
    logger.info(f"[DEBUG] Prob: {forensic_probability}, LIKELY_AI: {ScoringConfig.THRESHOLDS['LIKELY_AI']}, POSSIBLE: {ScoringConfig.THRESHOLDS['POSSIBLE_AI']}")
    ...
    logger.info(f"[DEBUG] Summary chosen: {summary}")
