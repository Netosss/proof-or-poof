...
def analyze_image_pro_turbo(image_source: Union[str, Image.Image]) -> dict:
    ...
    try:
        ...
        **STEP 1: WATERMARK & METADATA CHECK (The "Golden Gun")**
        *   **CRITICAL:** Scan the image **PIXEL-BY-PIXEL** for **SynthID** indicators for any known generated ai images watermarks.
        *   **IF FOUND (Even Faintly):**
            *   **STOP ANALYSIS** for that image. **IGNORE ALL VISUAL FLAWS.**

        
        **STEP 2: VISUAL ANALYSIS (Only if NO watermark found)**
        {quality_context}
        1.  **CONTEXT MATTERS:**
            *   **Art/Memes:** Be lenient. Do NOT flag stylized anatomy or "bad photoshop" edges as AI.
            *   **Photos:** Be strict. Look for "plastic/waxy" skin, merging objects, and dream-like physics.
        
        2.  **THE "TEXT" TRAP:**
            *   If you see text, **READ IT**. If the letters are gibberish/runes (e.g., "הצסיהת"), it is AI.
        
        ### OUTPUT DECISION:
        * **If AI (Watermark Found):**
            *   **Reason:** "Found AI generated details: SynthID watermark."
            *   **Confidence:** 1.0
        * **If AI (Visual Flaws Only):**
            *   **Reason:** [Max 7 words, e.g., "Menu text contains gibberish words"].
            *   **Confidence:** [0.85 - 0.98] (Reserve 1.0 for Watermarks)
        * **If NOT AI:**
...
def analyze_batch_images_pro_turbo(image_sources: list[Union[str, Image.Image]]) -> dict:
    ...
    try:
        ...
        # 3. PROMPT
        prompt_text = f"""
        Analyze EACH of the attached images INDEPENDENTLY for SYNTHETIC GENERATION ARTIFACTS.
        ...
        **STEP 1: WATERMARK & METADATA CHECK (The "Golden Gun")**
        *   **CRITICAL:** Scan EACH image **PIXEL-BY-PIXEL** for **SynthID** indicators.
        *   **LOOK FOR:** A small, white, four-pointed star icon (often in corners), "CR" icons, or DALL-E color strips.
        *   **IF FOUND (Even Faintly):**
            *   **STOP ANALYSIS** for that image. **IGNORE ALL VISUAL FLAWS.**
            *   **Explanation:** "Found AI generated details: SynthID watermark."
            *   **Confidence:** 1.0
        ...
        **STEP 2: VISUAL ANALYSIS (Only if NO watermark found)**
        ...
        - 0.85 - 0.98: Visible glitch (melting hands, gibberish text, asymmetrical pupils).
        - 1.00: **RESERVED FOR WATERMARKS/METADATA.**
        
        OUTPUT RULES:
        - If AI (Watermark): "Found AI generated details: SynthID watermark."
        - If AI (Visual): Explain in a professional understandable sentence (max 7 words).
        ...
        # Determine Majority
        # If even ONE image is high confidence AI (>= 0.95) or majority votes AI, flag it.
        if ai_votes and (len(ai_votes) >= len(not_ai_votes) or any(r["confidence"] >= 0.95 for r in ai_votes)):
            # AI wins
