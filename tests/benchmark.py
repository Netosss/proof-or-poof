...
def patch_detector_for_benchmark():
    ...
    async def mock_run_deep_forensics(file_path_or_img):
        ...
        return {
            ...
            # Keep 0ms so cost is $0 in any downstream logging; benchmark uses gpu_called to compute bypass.
            "gpu_time_ms": 0.0,
        }
...
async def main():
    ...
    for sig, n in summarize_common_signals(results, "fp")[:10]:
        print(f"  {n:>4}  {sig}")

    print("")
    print("Top FN signals:")
    for sig, n in summarize_common_signals(results, "fn")[:10]:
        print(f"  {n:>4}  {sig}")
    ...
    def show_examples(title: str, items: List[SampleResult]):
        ...
        for r in items[:10]:
            print(f"- {r.path}")
            print(f"  label={r.label} predicted={r.predicted} bypassed={r.bypassed} l2={r.l2_status} summary={r.summary}")
            if r.signals:
                print(f"  signals: {', '.join(r.signals[:6])}{' ...' if len(r.signals) > 6 else ''}")
